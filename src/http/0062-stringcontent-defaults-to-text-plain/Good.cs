...
var content = new StringContent(json, Encoding.UTF8, "application/json"); // labeled as JSON
...
if (response.StatusCode != HttpStatusCode.Created)
{
    throw new InvalidOperationException(
        $"the API rejected the POST with {(int)response.StatusCode} {response.StatusCode}");
}

Console.WriteLine("Order created. As it should be.");
